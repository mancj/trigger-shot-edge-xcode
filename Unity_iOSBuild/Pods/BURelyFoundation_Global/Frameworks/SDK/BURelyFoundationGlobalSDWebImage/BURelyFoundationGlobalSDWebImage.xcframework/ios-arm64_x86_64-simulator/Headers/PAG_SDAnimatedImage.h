...
* Original Files：SDAnimatedImage/SDWebImage (https://github.com/SDWebImage/SDWebImage)
...
/**
 This is the protocol for PAG_SDAnimatedImage class only but not for <PAG_SDAnimatedImageCoder>. If you want to provide a custom animated image class with full advanced function, you can conform to this instead of the base protocol.
 */
@protocol PAG_SDAnimatedImage <PAG_SDAnimatedImageProvider>
...
/**
 ...
 @param animatedCoder An animated coder which conform `<PAG_SDAnimatedImageCoder>` protocol
 @param scale The scale factor to assume when interpreting the image data. Applying a scale factor of 1.0 results in an image whose size matches the pixel-based dimensions of the image. Applying a different scale factor changes the size of the image as reported by the `size` property.
 @return An initialized object
 */
- (nullable instancetype)initWithAnimatedCoder:(nonnull id<PAG_SDAnimatedImageCoder>)animatedCoder scale:(CGFloat)scale;
 ...
 */
@interface PAG_SDAnimatedImage : UIImage <PAG_SDAnimatedImage>
 ...
 */
@property (nonatomic, assign, readonly) PAG_SDImageFormat animatedImageFormat;
